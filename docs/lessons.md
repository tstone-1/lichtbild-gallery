...
## Editor storage and save invariants
...
## Legacy shortcode takeover decisions
...
## Legacy record format and custom CSS
...
## Deployment transport and verification

Since the September 2026 review, chunks go to a unique sibling `.tmp` file. The uploader
checks its digest before replacing the live path with `RNFR`/`RNTO`, then checks the published
digest. An upload failure leaves the live file intact. Before any runtime upload, a disposable
two-file probe verifies that the server permits replacement by rename; failure aborts the
deployment. There is no delete-then-rename fallback. `tests/deploy-upload-test.sh` exercises
partial transfers, bad digests, rename refusal and the preflight call in `push`, offline.
This provides a file publication boundary; dependency order between different files still
matters, so run the server audit and plan for each release as described below.

Credential lookup is a gate too. A keychain refusal used to leave a non-empty partial `.netrc`,
which later requests reused, while the audit reported the failed reads as absent files. Failed
lookups now remove that partial file; the transport refuses to connect, and live audit and plan
stop before enumerating remote files. A connection failure is not evidence that a file is
absent either: `remote_size()` accepts only curl's remote-file-not-found status as absence,
and audit and plan stop on the first unreadable connection. The previous parser turned a
server closing the connection before its greeting into an all-files-absent audit with exit 0.
`tests/deploy-credentials-test.sh` exercises credential refusal, retry after unlocking,
successful caching, SIZE responses and both command entry points without a network connection.
An FTP login rejection also sets a per-run stop shared by every transport call, so chunk retries
and follow-up SIZE probes cannot repeat rejected credentials and trigger an IP ban.
When a password is supplied through a clipboard, capture it once and retain the successfully
tested value securely for the operation. Reading the clipboard again is a new observation,
not evidence that the same credential is being reused. Check transfer integrity without
printing the password, and remove temporary credential copies when the operation ends.
...
- *The 26.9.0 deploy, whose verification now refuses failed observations* — upload dependencies,
  archive checks and the two verification controls; full record in [docs/deploys.md](docs/deploys.md).
...
## Directory submission and publication history
...
alias or group address must allow-list `plugins@wordpress.org` too. The allow-list belongs in
the mailbox's spam filter, upstream of any rule in Apple Mail. A review thread that dies
